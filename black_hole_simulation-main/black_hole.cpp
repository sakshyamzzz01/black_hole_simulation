...
struct Engine {
  ...
  Engine() {
    ...
    if (!window) {
      cerr << "Failed to creatGLFW window\n";
      glfwTerminate();
      exit(EXIT_FAILURE);
    }
    ...
    this->earthShaderProgram = CreateEarthShaderProgram();
    initEarthOverlay();
  }
  ...
  void initEarthOverlay() {
    const int P = 18; // pea-sized
    float ndcW = 2.0f * P / float(WIDTH);
    float ndcH = 2.0f * P / float(HEIGHT);

    // bottom-right corner
    float quadVertices[] = {
        // positions                // texcoords
        1.0f - ndcW, -1.0f, 0.0f,        0.0f,         1.0f, -1.0f,
        1.0f,        0.0f,  1.0f,        -1.0f + ndcH, 1.0f, 1.0f,

        1.0f - ndcW, -1.0f, 0.0f,        0.0f,         1.0f, -1.0f + ndcH,
        1.0f,        1.0f,  1.0f - ndcW, -1.0f + ndcH, 0.0f, 1.0f};
  ...
  GLuint CreateEarthShaderProgram() {
    const char *vertexShaderSource = R"(
        #version 330 core
    ...
    const char *fragmentShaderSource = R"(
        ...
        void main() {
            ...
            float n1 = smoothNoise(centered * 3.0);
            float n2 = smoothNoise(centered * 7.0);
            float landMask = step(0.55, n1 * 0.6 + n2 * 0.4);

            vec3 ocean = vec3(0.0, 0.25, 0.55);
            vec3 land = vec3(0.1, 0.45, 0.15);
            vec3 earthColor = mix(ocean, land, landMask);

            float rim = pow(1.0 - r, 2.0);
            vec3 atmosphere = vec3(0.4, 0.7, 1.0) * rim * 0.6;

            vec3 finalColor = earthColor * (0.3 + 0.7 * diff) + atmosphere;
...
int main() {
  ...
  while (!glfwWindowShouldClose(engine.window)) {
    ...
    // Gravity
    if (Gravity) {
      std::vector<glm::vec3> accelerations(objects.size(), glm::vec3(0.0f));
      for (size_t i = 0; i < objects.size(); ++i) {
        for (size_t j = 0; j < objects.size(); ++j) {
          ...
          if (distance > 0) {
            double Gforce =
                (G * objects[i].mass * objects[j].mass) / (distance * distance);
            double acc_mag = Gforce / objects[i].mass;
            accelerations[i].x += (dx / distance) * acc_mag;
            accelerations[i].y += (dy / distance) * acc_mag;
            accelerations[i].z += (dz / distance) * acc_mag;
          }
        }
      }
      for (size_t i = 0; i < objects.size(); ++i) {
        objects[i].velocity.x += accelerations[i].x * dt;
        objects[i].velocity.y += accelerations[i].y * dt;
        objects[i].velocity.z += accelerations[i].z * dt;

        objects[i].posRadius.x += objects[i].velocity.x * dt;
        objects[i].posRadius.y += objects[i].velocity.y * dt;
        objects[i].posRadius.z += objects[i].velocity.z * dt;
        cout << "velocity: " << objects[i].velocity.x << ", "
             << objects[i].velocity.y << ", " << objects[i].velocity.z << endl;
      }
    }
    // ---------- GRID ------------- //
